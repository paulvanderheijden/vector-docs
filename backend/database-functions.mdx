...
Some fields are automatically populated by the backend during state transitions. These are enforced in DRF serializers, not in model `save()` methods.

### Decision

| Trigger | Field set | Value | Notes |
|---|---|---|---|
| Status → `Requested` | `requested_date` | Current date | Only if not already set. Starts the age counter. |

### Commitment

_No auto-set fields yet. Planned: auto-roll when due date passes (see below)._

<Note>
Auto-set fields are **not returned as separate changelog events** — they happen silently alongside the action that triggers them. The triggering action (e.g. status change) is logged normally.
</Note>
