...
| View restricted initiatives/projects | only if subscriber | only if subscriber | only if subscriber |
| Create project updates | yes (if project member) | yes (if project member) | yes (if project member) |
...
| Rule | Enforced by |
|---|---|
| A decision sponsor must be a member of the decision's circle | `sponsorPersonId` must be in `circle.members` |
...
| Person removed from workspace | Loses all access. All circle memberships are removed. Sponsorships and commitments should be reassigned. |
