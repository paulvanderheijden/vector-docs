...
The custom `User` extends `AbstractUser` with `username = None` and `USERNAME_FIELD = "email"`:
...
```
GET  /api/v1/{workspace_slug}/decisions/
POST /api/v1/{workspace_slug}/decisions/
GET  /api/v1/{workspace_slug}/decisions/{id}/
```

Two mechanisms enforce isolation:

1. **`HasWorkspaceAccess` permission** — checks that the authenticated user has an active `WorkspaceMember` record for the workspace in the URL
2. **`WorkspaceScopedViewSetMixin`** — auto-filters the queryset to only return rows belonging to the workspace, and auto-sets the workspace on create
...
The workspace slug appears in all domain API URLs. This is both a UX pattern (readable URLs) and a security mechanism — the `HasWorkspaceAccess` permission validates the slug against the user's memberships on every request.

```
/api/v1/mews/decisions/          → decisions in the "mews" workspace
/api/v1/mews/circles/            → circles in the "mews" workspace
/api/v1/mews/commitments/        → commitments in the "mews" workspace
```
