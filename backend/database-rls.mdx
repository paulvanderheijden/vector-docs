...
Every table in Vector has Row-Level Security (RLS) enabled. These policies are the **single source of truth** for authorization — the frontend doesn't need to implement access checks because Postgres enforces them on every query.
...
ALTER TABLE initiative_circles ENABLE ROW LEVEL SECURITY;
ALTER TABLE projects ENABLE ROW LEVEL SECURITY;
ALTER TABLE project_circles ENABLE ROW LEVEL SECURITY;
...
RETURNS boolean AS $$
  SELECT EXISTS (
    ...
    WHERE workspace_id = ws_id AND person_id = auth.uid() AND status = 'Active'
  );
...
RETURNS boolean AS $$
  SELECT EXISTS (
    SELECT 1 FROM workspace_members
    WHERE workspace_id = ws_id AND person_id = auth.uid() AND role = 'Admin' AND status = 'Active'
  );
$$ LANGUAGE sql SECURITY DEFINER STABLE;
...
Visible to all workspace members when no circles are linked. Restricted to linked circle members when circles are linked.

```sql
CREATE POLICY "Workspace members see initiatives" ON initiatives
  FOR SELECT USING (
    is_workspace_member(workspace_id)
    AND (
      NOT EXISTS (SELECT 1 FROM initiative_circles WHERE initiative_id = id)
      OR id IN (
        SELECT ic.initiative_id FROM initiative_circles ic
        JOIN circle_members cm ON ic.circle_id = cm.circle_id
...
Same visibility pattern as initiatives — public unless circles are linked.

```sql
CREATE POLICY "Workspace members see projects" ON projects
  FOR SELECT USING (
    is_workspace_member(workspace_id)
    AND (
      NOT EXISTS (SELECT 1 FROM project_circles WHERE project_id = id)
      OR id IN (
        SELECT pc.project_id FROM project_circles pc
...
CREATE POLICY "Project leads create updates" ON project_updates
  FOR INSERT WITH CHECK (
    project_id IN (
...
Workspace members can see activity events in their workspace. Events are insert-only — no updates or deletes.

```sql
CREATE POLICY "Workspace members see activity events" ON activity_events
  FOR SELECT USING (is_workspace_member(workspace_id));

CREATE POLICY "System inserts activity events" ON activity_events
