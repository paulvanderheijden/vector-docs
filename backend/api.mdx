...
All list endpoints return paginated responses using DRF's `PageNumberPagination` with a default page size of 50.
...
{
  "count": 127,
  "next": "https://api.example.com/api/v1/workspaces/mews/decisions/?page=2",
  "previous": null,
  "results": [ ... ]
}
```
...
Use the `page` query parameter to request a specific page: `?page=2`.

### Input sanitization

All rich text fields (`description`, `discussion`, `notes`, `content`) are sanitized server-side using [nh3](https://github.com/messense/nh3) to strip HTML tags. These fields store markdown — raw HTML is never allowed. Sanitization runs in each serializer's `validate_<field>` method before data reaches the database.
...
Update the current user's profile. Currently supports changing the UI language.
...
| Field | Type | Values |
|---|---|---|
| `ui_language` | string | `"en"` (English) or `"nl"` (Dutch) |
...
Domain endpoints follow a standard REST pattern, scoped to a workspace via the URL slug. All endpoints require authentication. Querysets are automatically scoped to the workspace via `WorkspaceScopedViewSetMixin`, which also verifies the user has an active membership in the workspace. `DELETE` operations require the `admin` role — regular members receive a `403 Forbidden` response.

### Soft delete

All `DELETE` requests perform a **soft delete** — the record's `deleted_at` timestamp is set rather than removing the row from the database. Soft-deleted records are automatically excluded from all API responses. This preserves data integrity, enables future recovery, and maintains referential consistency for related entities. The response remains `204 No Content`, so the behavior is transparent to API consumers.
