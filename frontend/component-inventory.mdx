...
Amber info banner with Lock icon. Used to indicate that a page or section is read-only (e.g. a closed session).
...
**Used in:** SessionDetail (shown when session status is Closed)
...
| `Badge` | Team chips, session badges, misc labels (status badges replaced by `StatusField`) |
