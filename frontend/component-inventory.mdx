...
Circle display: colored circle icon + name. Renders as `Link` (to circle sessions page) by default; set `linked={false}` for static display.
...
**Used in:** InitiativeDetail (target), ProjectDetail (visibility, owner)
