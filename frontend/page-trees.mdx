...
## Circle View `/circles/:slug/decisions` or `/circles/:slug/commitments`

Circle's decisions or commitments, section derived from URL path.

```
PageHeader (via usePageHeader)
├── PageHeaderRow
│   ├── PageHeaderGroup: circle icon + "Circle > Decisions" + decision filter tabs
│   └── PageHeaderGroup: [New Decision] button (decisions section only)
└── PageHeaderRow
    ├── PageHeaderGroup: Filter popover
    └── PageHeaderGroup: Display popover

Page body (conditional on section)
├── DecisionsTable (showCircle=false, showSession, noDividers)   ← decisions
└── CommitmentsTable (showCircle=false, noDividers)              ← commitments
```
...
└── PageHeaderRow
    ...
    └── PageHeaderGroup
        ├── EscalateDialog (mode="escalate")
        ├── EscalateDialog (mode="delegate")
