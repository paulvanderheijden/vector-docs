...
**Columns:** ref (e.g. DEC-1), statement (link or click handler), circle, session (link badge), owner (PersonField avatar), status (StatusField icon+text), age (colored by threshold).
...
| `combinedStatusOwner` | `boolean` | `false` | Merge StatusField + owner avatar into a single right-aligned column (hides individual status, owner, and age columns) |
...
- **PropertySelect** — click-to-open popover with options list. Used for health, target, visibility. Each option can have an icon. Shows checkmark on selected value. (Status fields now use the dedicated `StatusField` component.)
...
`WorkspaceProvider` wraps the entire app and stores the active workspace ID in `localStorage`. Pages call `useWorkspace()` to get `activeWorkspaceId` and pass it to workspace-scoped data helpers (e.g. `getCirclesForWorkspace`, `getInitiativesForWorkspace`).

```tsx
const { activeWorkspaceId, setActiveWorkspaceId, workspace, workspaces } = useWorkspace()
```

- **List pages** filter data by `activeWorkspaceId` — each data store exposes workspace-scoped accessors.
- **Detail pages** use ID-based lookups that search globally — IDs are unique across workspaces, so no filtering is needed.
