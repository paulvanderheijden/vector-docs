...
- **Right sidebar** — fixed-position 288px panel (or 480px for SessionDetail drawer), toggled via PanelRight icon button. Uses CSS transition on width for smooth slide. Contains `SidebarSection` with `PropertiesRow` (stacked) inside.
