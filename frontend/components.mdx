...
- **MarkdownEditor** (TipTap) — fires `onChange` on every keystroke → debounced PATCH via `useDebouncedSave` hook (800ms, flushes on unmount). The `tiptap-markdown` extension is configured with `html: false` to prevent raw HTML in markdown from being rendered as live DOM (XSS prevention).
