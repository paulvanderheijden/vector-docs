...
│   └── *.tsx            # Shared feature components (19 total)
...
└── lib/
    ...
    ├── projects-store.ts # Initiatives, projects, teams (separate store)
...
- **Inline editing** — titles, summaries, and properties are editable in place (InlineText + PropertySelect)
...
- **Two data stores** — `mock-data.ts` for decisions/commitments/circles/sessions, `projects-store.ts` for initiatives/projects/teams. These will converge when the backend is connected.
...
- **Detail sidebar is per-page** — each detail page renders its own fixed-position `<aside>`. There is no shared DetailSidebar wrapper component; only the inner `SidebarSection` and `PropertyRow` primitives are shared.
