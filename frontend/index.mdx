...
`WorkspaceProvider` wraps its context value in `useMemo` to prevent all consumers from re-rendering on unrelated parent state changes. Additional `React.memo` / `useMemo` should only be added after profiling with React DevTools confirms a measurable bottleneck.
